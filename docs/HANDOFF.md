...
最近已驗證部署的功能版本為 `a9daac0a0912de85864be4e184a2fc3e89630e9e`（第二十七批，2026-09-14）。VPS建置版本相同；兩站匯出已驗證。本文件後續提交不代表重新建置VPS映像。後續實作請同步維護 CHANGELOG.md 與本文件。
...
行情排程為唯一預設Hyperliquid市場快照抓取者；評分與WhaleAgent固定讀取本輪QuoteStore不可變副本。冷資料／過期不補抓、不投費率票。92項Python測試通過，提交不代表部署；仍需備份、預檢與新分析驗證。Leaderboard去重仍待處理。
