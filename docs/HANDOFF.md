...
最近已驗證部署的功能版本為 `8bbf9fc390bfa7312b7e1bc90dcbd6f645467f2e`（第二十八批，2026-09-14）。VPS建置版本相同；44個Python檔案簽章核對一致。本文件後續提交不代表重新建置VPS映像。後續實作請同步維護 CHANGELOG.md 與本文件。
...
Obsidian匯出改讀既有行情快照，跨所OI缺值不再退回HL；新增來源時間。88項Python測試通過。Hostinger主控台無回應後，經使用者確認改用一次性SSH金鑰部署。版本a9daac0、容器healthy；兩站35個讀取檢查200，兩站vault各40個幣別頁驗證通過，新分析於14:22:53 UTC完成。備份及金鑰清理狀態見本機outputs上線紀錄。

## 第二十八批已部署

行情排程為唯一預設Hyperliquid市場快照抓取者；評分與WhaleAgent固定讀取本輪QuoteStore不可變副本。冷資料／過期不補抓、不投費率票。92項Python測試通過；備份、8條接口與vault預檢成功後部署。新分析於14:43:24 UTC完成，178個評分費率與固定快照一致，取得時間1789396933.3289177；35條主要讀取檢查200，容器healthy。行情刷新曾失敗但保留未過期資料與原時間。Leaderboard去重仍待處理。
