...
@app.get("/account_activity")
def account_activity() -> dict:
    return dashboard_state().trader_summary.get("account_activity", {"groups":{}, "status":"waiting"})


@app.get("/account_history")
def account_history(address: str = Query(..., pattern="^0x[0-9a-fA-F]{40}$"),
                    cohort: str = Query("smart_verified", pattern="^(smart_verified|smart_pnl_only|whale)$")) -> dict:
    from pathlib import Path
    from ..storage.account_positions import AccountPositions
    orc = dashboard_state()
    group = orc.trader_summary.get("account_activity",{}).get("groups",{}).get(cohort,{})
    as_of = group.get("previous",{}).get("as_of")
    if not as_of: return {"history":[], "status":"waiting"}
    store = AccountPositions(Path(orc.config.posseries_db).parent / "account_positions.db")
